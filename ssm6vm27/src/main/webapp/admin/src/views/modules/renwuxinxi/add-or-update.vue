<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="警务号" prop="jingwuhao">
          <el-select  @change="jingwuhaoChange" v-model="ruleForm.jingwuhao" placeholder="请选择警务号">
            <el-option
                v-for="(item,index) in jingwuhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.jingwuhao" label="警务号" prop="jingwuhao">
          ...
          <el-form-item class="input" v-if="ruleForm.riqi" label="日期" prop="riqi">
              <el-input v-model="ruleForm.riqi" 
                placeholder="日期" readonly></el-input>
                                                                  ...
                                                                  <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="任务详情" prop="renwuxiangqing">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.renwuxiangqing" label="任务详情" prop="renwuxiangqing">
                    <span>{{ruleForm.renwuxiangqing}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                          ...
                                          this.$http({
              url: `option/jingwuyuan/jingwuhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.jingwuhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    jingwuhaoChange () {
      this.$http({
        url: `follow/jingwuyuan/jingwuhao?columnValue=`+ this.ruleForm.jingwuhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
