...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jingwuyuan');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','jingwuyuan');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('jingwuyuan', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussjingwuyuan/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('jingwuyuan', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('jingwuyuan', this.detail);
				this.$utils.msg('审核成功');
